...
const paymentMethods = [
  ...
  },
  {
    type: "CASH+CARD",
    addPayment: async (paymentModal: any, paymentAmount: string) => {
      const cleanTotal = paymentAmount.replace(/[^0-9.]/g, "")
      const totalAmount: number = parseFloat(cleanTotal)
      const cashAmount: string = (totalAmount * 0.6).toFixed(2)
      const cardAmount: string = (totalAmount * 0.4).toFixed(2)

      await paymentModal.addPaymentTender("CASH", cashAmount)
      await expect(paymentModal.tenderContainers.cash).toBeVisible()

      await paymentModal.addPaymentTender("CARD", cardAmount)
      await expect(paymentModal.tenderContainers.card).toBeVisible()
    },
    paymentTypeAssertion: (saleDoc: any) =>
      expect(saleDoc.paymentType).toBe("CARD"),
  ...
  {
    type: "CASH+STORECREDIT",
    addPayment: async (paymentModal: any, paymentAmount: string) => {
      const cleanTotal = paymentAmount.replace(/[^0-9.]/g, "")
      const totalAmount: number = parseFloat(cleanTotal)
      const cashAmount: string = (totalAmount * 0.8).toFixed(2)
      const storeCreditAmount: string = (totalAmount * 0.2).toFixed(2)

      await paymentModal.addPaymentTender("CASH", cashAmount)
      await expect(paymentModal.tenderContainers.cash).toBeVisible()

      await paymentModal.addPaymentTender("STORE_CREDIT", storeCreditAmount)
      await expect(paymentModal.tenderContainers.storecredit).toBeVisible()
    },
    paymentTypeAssertion: (saleDoc: any) =>
      expect(saleDoc.paymentType).toBe("CASH"),
  },
  {
    ...
    addPayment: async (paymentModal: any, paymentAmount: string) => {
      const cleanTotal = paymentAmount.replace(/[^0-9.]/g, "")
      const totalAmount: number = parseFloat(cleanTotal)
      const cardAmount: string = (totalAmount * 0.5).toFixed(2)
      const checkAmount: string = (totalAmount * 0.5).toFixed(2)

      await paymentModal.addPaymentTender("CARD", cardAmount)
      await expect(paymentModal.tenderContainers.card).toBeVisible()

      await paymentModal.addPaymentTender("CHECK", checkAmount)
      await expect(paymentModal.tenderContainers.check).toBeVisible()
    },
    paymentTypeAssertion: (saleDoc: any) =>
      expect(saleDoc.paymentType).toBe("CARD"),
...
test.describe("Basic sale tests", () => {
  test.beforeAll(() => {
  ...
  for (const payment of paymentMethods) {
    test(`User can make a sale with 100% ${payment.type}`, async ({
      home,
      paymentModal,
      context,
      page,
  ...
  test("User cannot start a sale with an empty cart", async ({
    home,
    paymentModal,
    page,
  }) => {
    ...
    await expect(home.alertMessage).toContainText(
      "There are no items in receipt"
    )
  })

  test("User can start a new sale by multiple ways", async ({
    home,
    page,
  }) => {
    await expect(home.userMenu).toBeVisible()
    ...
    await home.clickNewSale()
    await expect(home.confirmationModal).toBeVisible()
    await home.clickOk()

    await expect(home.shoppingCart).toBeHidden()
    await expect(home.cartTotal).toBeHidden()

    await home.userSearchesForCustomer(testCustomer)
    await home.userSearchesForProduct(cakeDetails.name.en)

    await home.clickBinSale()
    await expect(home.confirmationModal).toBeVisible()
    await home.clickOk()
